...
import { matchGroup, GROUP_JOIN_FEE, INDIVIDUAL_FEE } from '../lib/groups'
      ...
      {group && (
        <div className="border border-line border-l-4 border-l-marigold rounded-lg bg-white/70 p-7 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className="text-sm text-ink-soft">One fixed fee. No cut of your recovery.</span>
          </div>
        </div>
      )}

      {/* INDIVIDUAL FORK */}
      <p className="case-number text-seal text-sm mb-3">{group ? 'NOT SOLD ON THE GROUP?' : 'YOUR NEXT STEP'}</p>
      <div className="border border-line rounded-lg bg-white/70 p-6">
        <div className="flex flex-wrap justify-between items-center gap-5">
          <div className="max-w-xl">
            <p className="font-display text-xl text-ink mb-1">Prefer to pursue your own case?</p>
            <p className="text-sm text-ink-soft">
              File individually instead — draft a free pre-litigation notice, get a recovery
              assessment, and take it all the way to filing on your own terms.{' '}
              {group
                ? `A coordinated claim usually carries more weight, and going solo costs a little more (${INDIVIDUAL_FEE} vs ${GROUP_JOIN_FEE} to join) — but the choice is yours.`
                : `You only pay if the company ignores your free notice and you choose to file.`}
            </p>
          </div>
          <Link
            to="/individual-pursuit"
            className="whitespace-nowrap border border-line text-ink px-6 py-3 rounded-full font-medium hover:border-ink transition-colors"
          >
